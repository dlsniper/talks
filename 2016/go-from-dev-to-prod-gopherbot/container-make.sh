...
PROJECT_NAME='github.com/dlsniper/codemo'
PROJECT_DIR=${PWD}
...
docker run --rm \
        ...
        golang:1.7.1-alpine \
        go build -v -tags netgo -installsuffix netgo -ldflags "-X main.botVersion=${CONTAINER_TAG}" -o codemo ${PROJECT_NAME}
